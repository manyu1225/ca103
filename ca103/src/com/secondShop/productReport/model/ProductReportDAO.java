...
//	private static final String STATUS_REPORT = "Select from product_report where REPORT_STATUS =?";
	...
	public List<ProductReportVO> allProductReport( ) {
		...
		try {
			...
			Query<ProductReportVO> query = session.createQuery("from ProductReportVO",ProductReportVO.class);
	...
	public List<ProductReportVO> statusProductReport(Integer reportStatus) {
		...
		try {
			session.beginTransaction();
			Query<ProductReportVO> query = session.createQuery("from ProductReportVO where reportStatus=?0",ProductReportVO.class);
	...
	public void updateProductReport(ProductReportVO productReportVO) { //審核成功 狀1 去該販售商品會員的表格增加被檢舉次數1 檢舉成功商品改成狀態5
		...
		PreparedStatement pstmt = null;
				try {
					...
					System.out.println("productReportVO"+productReportVO.toString());
					con.commit();
				} catch (SQLException e) {
					if(con!=null) {
						try {
