...
<%
	//	這邊直接呼叫RouteCollectionService,跳過contorller
	//	當routeCollectionList==null代表第一次進入此頁面,所以自動撈出全部的路線
	//	當routeCollectionList!=null代表已經進入此頁面,所以把相關的查詢條件和查詢後的List設定好
	String hard, dis, slope, rot_loc, text, order, route_personal, route_game;
	List<RouteVO> routeCollectionList = (List<RouteVO>)session.getAttribute("routeCollectionList");
	...
	pageContext.setAttribute("routeCollectionList",routeCollectionList);
	...
	route_personal = (String)session.getAttribute("route_personal");
%>
	<br>
	<br>
	<div class="container">
		<div class="row">
			<div class="col">
	<img src="<%=request.getContextPath()%>/sources/img/Route/PersonalCollection4.png" style="width:100%;opacity:1">
	</div>
	</div>
	</div>
	<br>
...
<div class="container" style="margin-top:15px;">
		...
		<div class="col-auto">
			<ul class="nav nav-tabs" id="myTab" role="tablist" style="opacity:0.8">
				<li class="nav-item">
					<form action="<%=request.getContextPath()%>/RouteCollection/RouteCollectionServlet.do" Method="POST" name="route">
						<input type="hidden" value="getRouteCollectionList" name="action">
						<input type="hidden" value="route" name="order">
						<a class="nav-link ${('route'==route_order || (null == route_order && null == route_game && null == route_personal))?'active':'' }" id="home-tab" href="javascript:document.route.submit();" role="tab" aria-controls="home" aria-selected="true">全部路線</a>
					</form>
				</li>
				<li class="nav-item">
					<form action="<%=request.getContextPath()%>/RouteCollection/RouteCollectionServlet.do" Method="POST" name="route_game">
						<input type="hidden" value="getRouteCollectionList" name="action">
						<input type="hidden" value="route_game" name="route_game">
				...
				<li class="nav-item">
					<form action="<%=request.getContextPath()%>/RouteCollection/RouteCollectionServlet.do" Method="POST" name="route_not_game">
						<input type="hidden" value="getRouteCollectionList" name="action">
				...
				<li class="nav-item">
					<form action="<%=request.getContextPath()%>/RouteCollection/RouteCollectionServlet.do" Method="POST" name="route_personal">
						<input type="hidden" value="getRouteCollectionList" name="action">
						<input type="hidden" value="route_personal" name="route_personal">
						<input type="hidden" value="${ route_order }" name="route_order">
						<input type="hidden" value="${ (hard == null)?'>-100':hard }" name="hard">
						<input type="hidden" value="${ (dis == null)?'>-100':dis }" name="dis">
						<input type="hidden" value="${ (slope == null)?'>-100':slope }" name="slope">
						<input type="hidden" value="${ (rot_loc == null)?'':rot_loc }" name="rot_loc">
						<input type="hidden" value="${ (text == null)?'':text }" name="text">
						<a class="nav-link ${('route_personal'==route_personal)?'active':'' }" id="contact-tab" href="javascript:document.route_personal.submit();" role="tab" aria-controls="home" aria-selected="false">私人路線</a>
					</form>
				</li>
			</ul>
			</div>
			...
			<ul class="nav nav-tabs" id="myTab" role="tablist" style="opacity:0.8">
				<li class="nav-item">
					<form action="<%=request.getContextPath()%>/RouteCollection/RouteCollectionServlet.do" Method="POST" name="route_popu">
						<input type="hidden" value="getRouteCollectionList" name="action">
				...
				<li class="nav-item">
					<form action="<%=request.getContextPath()%>/RouteCollection/RouteCollectionServlet.do" Method="POST" name="route_hard">
						<input type="hidden" value="getRouteCollectionList" name="action">
				...
				<li class="nav-item">
					<form action="<%=request.getContextPath()%>/RouteCollection/RouteCollectionServlet.do" Method="POST" name="route_dis">
						<input type="hidden" value="getRouteCollectionList" name="action">
				...
				<li class="nav-item">
					<form action="<%=request.getContextPath()%>/RouteCollection/RouteCollectionServlet.do" Method="POST" name="route_slope_ave">
						<input type="hidden" value="getRouteCollectionList" name="action">
...
<%@ include file="/sources/file/Route/RouteCollectionListPageTop.file" %> 
<c:if test="${ routeCollectionList.size()>0 }">
<% int routeRanking = 0; %>
	<c:forEach var="rotVO" items="${routeCollectionList}" begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">
<% routeRanking++; %>
      ...
      <div class="col-7" style="padding:5px;height:170px ">
        ...
        <div class="container">
          <div class="row" style="height:65px; margin-top:5px;">
            ...
            <div class="col-2 text-right" style="padding:0px;font-size:70px; margin-top:45px; color:#91e6f9;opacity:0.3;text-shadow: black 0.1em 0.1em 0.2em font-family: Comic Sans MS,arial,helvetica,sans-serif;" ></div>
          </div>
        </div>
...
<%@ include file="/sources/file/Route/RouteListPageBottom.file" %> 
	...
	<div id="MyBlog">
		...
		<a href="#pageTop">Personal Collection</a>
	</div>
