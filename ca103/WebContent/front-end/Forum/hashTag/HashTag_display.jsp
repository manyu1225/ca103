...
<jsp:useBean id="forPosSvc2" scope="page" class="com.forPos.model.Forum_post_Service" />
...
Set<String> listAllTag = hashTagSvc.getAllPostTag();  //為了要求不重複
...
<!-- //  List<String> listAllTag = hashTagSvc.getAllPostTag(); -->
...
<!-- 			顯示全部標籤選項 -->
			...
			<div class="col-md-9 col-12 mt-5">
				...
				<section id="pinBoot">
					...
					<c:forEach var="forPosID_Str" items="${listPostByTag}" >
						<article class="white-panel">
							<img id="thumbnail" src="<%=request.getContextPath()%>/forPos/forPos.do?action=getOnePic&forPost_ID=${forPosID_Str}" style="width: 350px; height: 200px" alt="">
										
							<h4>
								<a id="forPost_theme"
									href="<%=request.getContextPath()%>/forPos/forPos.do?forPost_ID=${forPosID_Str}&mem_ID=${forPosSvc2.getOneForPos(forPosID_Str).mem_ID}&action=getOnePos_onPage&forPost_state=${forPosSvc2.getOneForPos(forPosID_Str).forPost_state}&forPost_view=${forPosSvc2.getOneForPos(forPosID_Str).forPost_view}&actionTag=getHashTag">
									${forPosSvc2.getOneForPos(forPosID_Str) == null ? '此篇文章已不存在': forPosSvc2.getOneForPos(forPosID_Str).forPost_theme}</a>
							</h4>
							<!-- 						內容 -->
							<!-- 						<div class="box"> -->
							<%-- 						<p class="JQellipsis">${forPosVO.forPost_content}</p> --%>
							<!-- 						</div> -->


							<p>
								<i class="far fa-eye"></i>${forPosSvc2.getOneForPos(forPosID_Str).forPost_view}</p>			
